...
async def read_table(page) -> list[dict]:
    stocks = []
    ...
    for row in rows:
        ...
        values = []
        for cell in cells:
            text = (await cell.inner_text()).strip().split("\n")[0].strip()
            values.append(text)
        if not values:
            continue
        if not headers and len(values) > 5 and values[0] == "Ticker":
            headers = values
            print(f"Headers: {headers[:6]} ({len(headers)} total)")
            continue
...
def analyze(stocks: list[dict]) -> list[dict]:
    if not API_KEY:
    ...
    prompt = (
        f"You are an expert CANSLIM analyst screening for multi-bagger opportunities.\n\n"
        f"Top {len(stocks)} stocks from CANSLIM screener (sorted by N-Value, max 8190):\n"
        + "\n".join(lines) +
        ...
        "  \"bull_case\": \"5 sentences.\",\n"
        "  \"key_risk\": \"2 sentence.\",\n"
        "  \"price_target_pct\": 85,\n"
        "  \"catalyst\": \"next specific catalyst\"\n"
        "}]\n"
        "verdict: STRONG BUY / BUY / SPECULATIVE BUY | conviction: 1-10 | price_target_pct: % upside over 1-2 years"
    )
    ...
    result = []
    for pick in picks:
        base = ticker_map.get(pick.get("ticker",""), {
            "ticker":pick.get("ticker","?"),"nval":0,"lwowski":0,"primaryTests":0,
            "rsVal":0,"epsPct":0,"salesPct":0,"instOwn":0,"price":0,"high52":0,
            "low52":0,"volume":0,"sma200pct":0,"sma50_150":False,"sma150_200":False,
    ...
    print(f"After conviction filter (>=7): {len(result)} stocks")
    return result[:TOP_N]


def build_dashboard(top, total, timestamp):
    if not top:
        return "<html><body><h1>No stocks passed the conviction filter.</h1></body></html>"
    ...
    LABELS = ["SMA50>150","SMA150>200","52wk Span","RS","Liquidity","Near 52wH",
              "Prev Close","SMA200up","Inst Own",">SMA50","Sales","EPS"]
    ...
    for s in top:
        ...
        pills = "".join(
            f'<span class="pill {"pass" if s.get(r) else "fail"}">{lb}</span>'
            for r,lb in zip(RULES,LABELS)
        )

        cards += (
            f'<div class="card" data-verdict="{s.get("verdict","N/A")}">' +
            f'<div class="ch"><div class="cl"><div class="rn">#{s.get("rank","?")}</div>' +
            f'<div><div class="tn">{s.get("ticker","?")}</div><div class="tl">{s.get("theme","")}</div></div></div>' +
            f'<div class="cr"><span class="vc" style="color:{col};background:{col}18;border:1.5px solid {col}40">{s.get("verdict","N/A")}</span>' +
            f'<div class="ul" style="color:{pc(up)}">{up:+}% target</div></div></div>' +
            f'<div class="mg">' +
            f'<div class="mb"><div class="ml">N-Value</div><div class="mv" style="color:#d97706">{n:,}</div><div class="nb"><div style="width:{np}%;background:#d97706;height:100%;border-radius:2px"></div></div></div>' +
            f'<div class="mb"><div class="ml">Lwowski</div><div class="mv" style="color:#7c3aed">{lw:,}</div></div>' +
            f'<div class="mb"><div class="ml">Tests</div><div class="mv" style="color:#16a34a">{tests}/11</div></div>' +
            f'<div class="mb"><div class="ml">RS Value</div><div class="mv" style="color:#2563eb">{rs}</div></div>' +
            f'<div class="mb"><div class="ml">EPS Growth</div><div class="mv" style="color:{pc(eps)}">{eps:+.1f}%</div></div>' +
            f'<div class="mb"><div class="ml">Sales Growth</div><div class="mv" style="color:{pc(sales)}">{sales:+.1f}%</div></div>' +
            f'<div class="mb"><div class="ml">Inst. Own</div><div class="mv" style="color:#7c3aed">{inst:.1f}%</div></div>' +
            f'<div class="mb"><div class="ml">Price</div><div class="mv">${price:.2f}</div></div>' +
            f'<div class="mb"><div class="ml">52w High</div><div class="mv">${h52:.2f}</div></div>' +
            f'<div class="mb"><div class="ml">52w Low</div><div class="mv">${l52:.2f}</div></div>' +
            f'<div class="mb"><div class="ml">vs 52w High</div><div class="mv" style="color:{pc(ph-100)}">{ph}%</div></div>' +
            f'<div class="mb"><div class="ml">Volume</div><div class="mv">{vol/1e6:.1f}M</div></div>' +
            f'</div>' +
            f'<div class="rs"><div class="rt">CANSLIM Rules</div><div class="rp">{pills}</div></div>' +
            f'<div class="ag">' +
            f'<div class="ab bull"><b class="ai">+</b><div><div class="al">Bull Case</div><div class="at">{s.get("bull_case","")}</div></div></div>' +
            f'<div class="ab bear"><b class="ai">-</b><div><div class="al">Key Risk</div><div class="at">{s.get("key_risk","")}</div></div></div>' +
            f'<div class="ab cat"><b class="ai">*</b><div><div class="al">Catalyst</div><div class="at">{s.get("catalyst","")}</div></div></div>' +
            f'</div>' +
            f'<div class="convrow"><span class="convlbl">Conviction</span>' +
            f'<div class="convbar"><div style="width:{conv*10}%;background:{cc};height:100%;border-radius:3px"></div></div>' +
            f'<span class="convnum">{conv}/10</span></div>' +
            f'</div>'
        )

    an  = sum(s.get("nval",0) for s in top)//(len(top) or 1)
    aup = sum(s.get("price_target_pct",0) for s in top)//(len(top) or 1)
    aep = sum(s.get("epsPct",0) for s in top)//(len(top) or 1)
...
.lt{font-size:16px;font-weight:700;color:#111827;letter-spacing:-0.02em;}.lt span{color:#2563eb;}
...
.tb{padding:0 32px 16px;display:flex;align-items:center;gap:8px;flex-wrap:wrap;}
...
.pill{font-size:10px;padding:3px 7px;border-radius:4px;font-weight:500;font-family:monospace;}
.pill.pass{background:#dcfce7;color:#15803d;border:1px solid #bbf7d0;}
.pill.fail{background:#f9fafb;color:#9ca3af;border:1px solid #e5e7eb;}
...
.foot{padding:20px 32px;text-align:center;font-size:11px;color:#9ca3af;border-top:1px solid #e5e7eb;background:#fff;}
.foot a{color:#2563eb;text-decoration:none;font-weight:500;}
@media(max-width:640px){.grid{grid-template-columns:1fr;padding:0 16px 28px;}.mg{grid-template-columns:repeat(3,1fr);}.sum{grid-template-columns:repeat(2,1fr);padding:16px;}.header,.tb{padding-left:16px;padding-right:16px;}}
"""
...
<title>CANSLIM AI Screener - {timestamp}</title>
...
<header class="header">
  ...
  <div class="hm"><span class="ld"></span>Live from canslimscreener.com · Claude AI<br><strong>{total}</strong> stocks screened · {timestamp}</div>
</header>
<div class="sum">
  <div class="sc"><div class="sv" style="color:#2563eb">{len(top)}</div><div class="sl">AI Top Picks</div><div class="ss">conviction &ge; 7/10</div></div>
  <div class="sc"><div class="sv" style="color:#d97706">{an:,}</div><div class="sl">Avg N-Value</div><div class="ss">max 8,190</div></div>
  <div class="sc"><div class="sv" style="color:#16a34a">{aep:+}%</div><div class="sl">Avg EPS Growth</div><div class="ss">quarterly YoY</div></div>
  <div class="sc"><div class="sv" style="color:#7c3aed">{aup:+}%</div><div class="sl">Avg AI Upside</div><div class="ss">1-2 year target</div></div>
</div>
<div class="tb">
  <span class="tbl">Filter:</span>
  <button class="fb on" onclick="f('all',this)">All Picks</button>
  <button class="fb" onclick="f('STRONG BUY',this)">Strong Buy</button>
  <button class="fb" onclick="f('BUY',this)">Buy</button>
  <button class="fb" onclick="f('SPECULATIVE BUY',this)">Speculative</button>
</div>
...
async def main():
    ...
    print("\nStep 2/3 — Claude AI analysis...")
    top = analyze(stocks)
    if not top:
        print("No stocks passed conviction filter.")
        sys.exit(1)
    print("\nStep 3/3 — Building dashboard...")
    ts = datetime.now().strftime("%B %d, %Y  %H:%M")
    OUTPUT.write_text(build_dashboard(top, len(stocks), ts), encoding="utf-8")
    print(f"Saved to {OUTPUT.resolve()}")
    print("\nTop picks:")
    for s in top:
        print(f"  #{s.get('rank','?')} {s['ticker']:6s} {s.get('verdict','?hj'):15s} EPS:{s.get('epsPct',0):+.0f}% conv:{s.get('conviction',0)}/10")
